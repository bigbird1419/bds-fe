...
function FillterItem({ icon, head = 'default', type, data = [], onClick = () => { } }) {
    ...
    return (
        <div className="h-full bg-white p-2 mr-2 rounded-md cursor-pointer text-gray-500 text-sm border-1 border-gray-200 hover:border-gray-400" onClick={() => setIsShowPopup(true)}>
            ...
            {isShowPopup &&
                <div className="fixed bg-gray-800 bg-opacity-80 top-0 left-0 right-0 bottom-0 z-30" onClick={e => handleChildClick(e)}>
                    <div className="absolute left-1/2 top-1/2 translate-x-[-50%] translate-y-[-50%] w-3/6 h-4/5 bg-white rounded-md overflow-hidden z-50">
                        <div className="relative">
                            <div className="absolute top-1/2 left-0 translate-y-[-50%] ml-2 text-colorPrimary text-xl  cursor-pointer hover:bg-gray-100 px-3 py-2 rounded-md" onClick={e => handleChildClick(e)}><i className="fa-solid fa-angle-left"></i></div>
